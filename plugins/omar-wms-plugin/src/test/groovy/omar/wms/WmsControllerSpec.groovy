...
class WmsControllerSpec extends Specification
{
    ...
    void "test getMap() with format=image/vnd.jpeg-png"()
    {
        def wms = Spy(WebMappingService)

        final GetMapRequest exampleAutoFormatRequest = new GetMapRequest()

        exampleAutoFormatRequest.with {
            width = 512
            height = 512
            service = "WMS"
            version = "1.1.1"
            ...
            srs = "EPSG:4326"
            crs = "ESPG:4326"
            bbox = '-3.6177,40.4172,-3.5438,40.4663'
            format = 'image/vnd.jpeg-png'
            layers = 'omar:raster_entry.5802'
            styles = null
            transparent = false
            filter = null
            exceptions = null
        ...
        when:
            wms.getMap(exampleAutoFormatRequest, false)

        then:
            1 * wms.parseLayers(exampleAutoFormatRequest) >> parseLayersResult
            1 * wms.parseBbox(exampleAutoFormatRequest) >> parseBboxResult
            1 * wms.callOmsService(callOmsServiceInput) >> callOmsServiceResult
    }
}
